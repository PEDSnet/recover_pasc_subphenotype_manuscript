...
get_cohort_sentences<-function(cohort, r1=-15, r2=15, incident=FALSE, washout_months=18){
  ...
  if (incident==TRUE){
    cond_codes_washout<-cond_tbl %>%
      filter(condition_start_date<observation_date-days(7), condition_start_date>observation_date-months(washout_months)) %>% 
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id=="ICD10CM") %>% select(concept_id), by=c("condition_source_concept_id"="concept_id")) %>%
      rename(concept_id=condition_source_concept_id) %>% 
      distinct(person_id, concept_id) %>% 
      compute_new(indexes=list(c("person_id")))
      
    cond_codes<-cond_tbl %>% 
      filter(condition_start_date>observation_date+days(r1), condition_start_date<=observation_date+days(r2)) %>%
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id=="ICD10CM") %>% select(concept_id), by=c("condition_source_concept_id"="concept_id")) %>%
      rename(concept_id=condition_source_concept_id) %>% 
      distinct(person_id, concept_id, site) %>% 
      anti_join(cond_codes_washout, by=c("person_id", "concept_id")) %>% 
      compute_new(indexes=list(c("person_id")))
    
    drug_codes_washout<-drug_tbl %>%     
      filter(drug_exposure_start_date<observation_date-days(7), drug_exposure_start_date>observation_date-months(washout_months)) %>%
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id=="RxNorm", domain_id=="Drug") %>% select(concept_id), by=c("drug_concept_id"="concept_id")) %>%   
      rename(concept_id=drug_concept_id) %>% 
      distinct(person_id, concept_id) %>% 
      compute_new(indexes=list(c("person_id")))
    
    drug_codes<-drug_tbl %>%     
      filter(drug_exposure_start_date>observation_date+days(r1), drug_exposure_start_date<=observation_date+days(r2)) %>%
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id=="RxNorm", domain_id=="Drug") %>% select(concept_id), by=c("drug_concept_id"="concept_id")) %>%   
      rename(concept_id=drug_concept_id) %>% 
      distinct(person_id, concept_id, site) %>%
      anti_join(drug_codes_washout, by=c("person_id", "concept_id")) %>% 
      compute_new(indexes=list(c("person_id")))
    
    proc_codes_washout<-proc_tbl %>% 
      filter(procedure_date<observation_date-days(7), procedure_date>observation_date-months(washout_months)) %>%    
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id %in% c("ICD10PCS", "HCPCS", "CPT4"), domain_id=="Procedure") %>% select(concept_id), by=c("procedure_concept_id"="concept_id")) %>%
    ...
    proc_codes<-proc_tbl %>% 
      filter(procedure_date>observation_date+days(r1), procedure_date<=observation_date+days(r2)) %>%    
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id %in% c("ICD10PCS", "HCPCS", "CPT4"), domain_id=="Procedure") %>% select(concept_id), by=c("procedure_concept_id"="concept_id")) %>%
      rename(concept_id=procedure_concept_id) %>% 
      distinct(person_id, concept_id, site) %>%
      anti_join(proc_codes_washout, by=c("person_id", "concept_id")) %>% 
      compute_new(indexes=list(c("person_id")))
    
    lab_codes_washout<-lab_tbl %>% 
      filter(measurement_date<observation_date-days(7), measurement_date>observation_date-months(washout_months)) %>%  
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id %in% c("LOINC"), domain_id=="Measurement") %>% select(concept_id), by=c("measurement_concept_id"="concept_id")) %>%
      rename(concept_id=measurement_concept_id) %>% 
      distinct(person_id, concept_id) %>% 
      compute_new(indexes=list(c("person_id")))
    
    lab_codes<-lab_tbl %>% 
      filter(measurement_date>observation_date+days(r1), measurement_date<=observation_date+days(r2)) %>%  
      inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id %in% c("LOINC"), domain_id=="Measurement") %>% select(concept_id), by=c("measurement_concept_id"="concept_id")) %>%
      rename(concept_id=measurement_concept_id) %>% 
      distinct(person_id, concept_id, site) %>% 
      anti_join(lab_codes_washout, by=c("person_id", "concept_id")) %>% 
      compute_new(indexes=list(c("person_id")))
    
    n_codes=cond_codes %>%
      union_all(drug_codes) %>%
      union_all(proc_codes) %>%
      union_all(lab_codes) %>% 
...
get_cohort_sentence_names<-function(cohort, r1=-15, r2=15){
  ...
  cond_codes<-cond_tbl %>% 
    filter(condition_start_date>observation_date+days(r1), condition_start_date<=observation_date+days(r2)) %>%
    inner_join(vocabulary_tbl("concept") %>% filter(concept_code!="No matching concept", vocabulary_id=="ICD10CM") %>% select(concept_id, concept_name), by=c("condition_source_concept_id"="concept_id")) %>%
    rename(concept_id=condition_source_concept_id) %>% 
    distinct(person_id, concept_id, site) %>% 
    compute_new(indexes=list(c("person_id")))
...
#  n_codes=n
  ...
  code_tbl<-cond_codes %>%
    ...
    compute_new(indexes=list(c("person_id", "concept_id")))
...
get_covid_positives<-function(observation_derivation_recover=cdm_tbl("observation_derivation_recover"),
  ...
  covid_dx_temp<-covid_dx_specific %>%
    ...
    compute_new(indexes=list(c("person_id", "visit_occurrence_id", "observation_id")))
  ...
  positive_first<-positive_non_misc_cohort %>% 
    ...
    compute_new(indexes=list(c("person_id", "visit_occurrence_id")))
...
get_serology_positives<-function(observation_derivation_recover=cdm_tbl("observation_derivation_recover"), max_date="2022-09-01"){
  ...
  serology_filtered<-serology %>%
    ...
    filter(include_flag==1) %>% distinct(observation_id) %>% compute_new(indexes=list("observation_id"))
...
get_b94_dx<-function(
    b94_src_val_exclude=read.csv("./specs/b94_src_val_exclude.csv") %>% 
      ...
      output_tbl("b94_src_val_exclude", temp=TRUE, indexes=list("condition_source_value"))
){
...
require_prior_encounter<-function(cohort){
  ...
  cohort %>% inner_join(keep_ids, by="person_id") %>% compute_new(indexes=list("person_id"))
}
...
get_cluster_visits<-function(cohort, cond_codeset=load_codeset("cluster_master_pasc")){
  ...
  cohort %>% left_join(cohort_clust_occurrence, by="person_id") %>%
    replace_na(myList) %>%compute_new(indexes=list("person_id"))
...
get_util<-function(cohort,
                   visit_tbl=cdm_tbl("visit_occurrence") %>% dplyr::select(visit_start_date, person_id, site)
){
  cohort_util<-cohort %>% 
    ...
    compute_new(indexes=list("person_id", "site"))
  
  cohort %>%
    left_join(cohort_util, by=c("person_id", "site"))
...
find_icu <- function(adt_tbl = cdm_tbl('adt_occurrence'),
                     cohort_tbl) {
  cohort_tbl %>% select(person_id, observation_date) %>%
    inner_join(adt_tbl, by = 'person_id') %>%
    filter(service_concept_id %in% c(2000000079L,2000000080L,2000000078L)) %>%
    select(person_id, adt_date, observation_date, adt_type_concept_id) 
}
...
find_hosp <- function(visit_tbl = cdm_tbl('visit_occurrence'),
  ...
  cohort_tbl %>% select(person_id, observation_date) %>%
    ...
    select(person_id, visit_start_date, observation_date, visit_concept_id)
}
