  ...
  only_pmca_conds <- 
    ...
    inner_join(
      select(condition_tbl,
             ...
             condition_start_date),
      by=c('person_id', 'site')
    ) %>% inner_join(visit_tbl %>% select(visit_occurrence_id, site, visit_start_date), by=c("visit_occurrence_id", "site"))%>% 
...
compute_pmca_summary <- function(pmca_lookup_tbl) {
  ...
  all_pts <- 
    dplyr::union(
      add_year %>% ungroup() %>% select(person_id, site),
      progressive_malignant_pts %>% select(person_id, site)
    )
...
compute_pmca_cats_cons <- function(pmca_summary_tbl) {
  ...
  complex_pts <- 
    dplyr::union(
      gt_two_bs %>% select(person_id,cohort_entry_date),
      prog_or_malig %>% select(person_id,cohort_entry_date)
...
compute_pmca_cats_lib <- function(pmca_summary_tbl) {
  ...
  complex_pts <- 
    dplyr::union(
      gt_two_bs %>% select(person_id,site, cohort_entry_date),
      prog_or_malig %>% select(person_id, site, cohort_entry_date)
    ) %>% mutate(complex_chronic = 1L) %>% 
    compute_new(temporary=TRUE,
                indexes=list('person_id', 'site'))
